const BankModel = require('./bank.model');
  ...
  exports.list = (req, res ) => {
      ...
      BankModel.list(limit, page,req.query)
          .then((result) => {
...
exports.listSuggestions = (req, res ) => {
     ...
     */
    BankModel.listSuggestions(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
  ...
  exports.getById = (req, res) => {
     ...
     */
    BankModel.findById(req.params.bankId,filter)
          .then((result) => {
              res.status(200).send(result);
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.patchById = (req, res) => {
     ...
     */
      BankModel.patchBank(req.params.bankId, req.body,filter)
          .then((result) => {
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.removeById = (req, res) => {
     ...
     */
    BankModel.removeById(req.params.bankId,filter)
          .then((result)=>{
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
