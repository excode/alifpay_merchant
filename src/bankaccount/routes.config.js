...
{ctrl:'acc_desc',format:'text',required:true},
  ...
  exports.routesConfig = function (app) {
      ...
      app.post('/bankaccount/upload/:columnName/:rowId', [
        ValidationMiddleware.validJWTNeeded,
      ...
      app.get('/bankaccount/all', [   //  Required to Fill UI Component like Dropdown ,List , can be disabled if not required
        ValidationMiddleware.validJWTNeeded,
        PermissionMiddleware.minimumPermissionLevelRequired(USER),
        BankaccountController.listAll
    ]);
    app.get('/bankaccount/suggestions', [   //  Required to Fill UI Component like Auto Complete , can be disabled if not required
        ValidationMiddleware.validJWTNeeded,
        PermissionMiddleware.minimumPermissionLevelRequired(USER),
        BankaccountController.listSuggestions
    ]);
      app.get('/bankaccount/:bankaccountId', [
          ValidationMiddleware.validJWTNeeded,
          PermissionMiddleware.minimumPermissionLevelRequired(USER),
          BankaccountController.getById
      ]);
      app.patch('/bankaccount/:bankaccountId', [
          ValidationMiddleware.validJWTNeeded,
          PermissionMiddleware.minimumPermissionLevelRequired(USER),
          FormValidation.formValidation(formValidationRules,'UPDATE'),
          BankaccountController.patchById
      ]);
      app.delete('/bankaccount/:bankaccountId', [
          ValidationMiddleware.validJWTNeeded,
